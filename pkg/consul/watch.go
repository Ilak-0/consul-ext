...
// NewWatch returns a new Watcher
func NewWatch(consulClient *consul.Client, retryTime time.Duration, debounceTime time.Duration) *Watcher {
...
func (w *Watcher) WatchTree(ctx context.Context, path string) (<-chan consul.KVPairs, error) {
	...
	_, meta, err := kv.List(path, opts.WithContext(ctx))
	if err != nil {
		return nil, err
	}
		...
		for {
			...
			if opts.WaitIndex != meta.LastIndex {
				...
				if opts.WaitIndex <= 0 ||
					...
					out <- NewKVPairsDataFilter(kvPairs, opts.WaitIndex)
					debounceStart = time.Time{}
				} else {
					if debounceStart.IsZero() {
						debounceStart = time.Now()
					}
					lastIndex := opts.WaitIndex
					debounceTimer = time.AfterFunc(w.debounceTime, func() {
						select {
						case out <- NewKVPairsDataFilter(kvPairs, lastIndex):
						case <-ctx.Done():
							return
						}
						debounceStart = time.Time{}
...
func NewKVPairsDataFilter(kvs []*consul.KVPair, lastIndex uint64) []*consul.KVPair {
	var newKVPairs []*consul.KVPair
	for _, kv := range kvs {
		if kv.ModifyIndex > lastIndex {
